...
Functional eval: agent applied each created skill to the brief's eval prompt; responses in `eval/response.md`. Graded with **agent grader** (`agents/grader.md`) — see `eval/grading.json` per response and `report-grader.json`.
...
| Agent grader pass rate | 0.889 | 1.0 | **v5** |
| Head-to-head briefs | 0/3 | 3/3 | **v5** |

**Verdict: v5_wins** (structural + agent grader)

## Per-brief highlights

| Brief | Structural Δ (v5−v4) | Agent grader v4 / v5 |
|-------|---------------------|----------------------|
| code-review | +8.4 | 1.0 / 1.0 |
| commit-message | +7.8 | 1.0 / 1.0 |
| grill-me-clone | +11.4 | 0.667 / 1.0 |
...
```bash
python bakeoff/run_live_bakeoff.py
